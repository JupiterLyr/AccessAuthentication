...
1. Create a TXT file and write the configuration you want to register. Details are in the next section. The software needs to be verified according to the configuration you set.
...
<br></br>
<h2>How to customize the registration?</h2>
...
2. The content of the TXT file contains an identifier, a password, and a folder path that jumps after verification, which are separated by a symbol `|` with no spaces on both sides, i.e.:
```
...
1. The separator of the 3 parts of information is a vertical line `|`, and each line is written with only one data label. Its standard format is `Identifier|Password|Folder_Path`. The folder path can be copied directly from the explorer. If the folder is placed in a removable storage medium (CD-ROM, USB flash drive, removable hard disk, etc.), please manually change the drive letter to `Z:`, and the software needs to run on this storage medium. Because `Z:` will be redirected to the drive letter where the software runs. E.g.:
```txt
...
1. 首先创建一个 TXT 文件，写入想注册的配置，具体步骤见下一节。软件需要根据你设定的配置进行验证。
...
2. TXT 文件的内容包含标识符、密码、验证后跳转的文件夹路径，两两用符号 `|` 分隔，注意符号两侧不加空格，即：
```
        标识符|密码|文件夹路径
```
...
1. 三部分信息的分隔符为半角竖线 `|`，每行写入一个数据标签，其标准格式为：`标识符|密码|跳转路径`。跳转路径直接从文件夹路径处复制即可；若文件夹放置于可移动存储媒介（光盘、U盘、移动硬盘等）中，请将盘符手动改为 `Z:`，同时该软件需要在该存储媒介上运行，因为 `Z:` 会重定向至软件运行目录所在盘符。例如：
```txt
